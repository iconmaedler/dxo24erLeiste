            ...
            Section("Levels") {
                ...
                HStack {
                    Slider(value: Binding(
                        ...
                        set: { device.inputLevel = $0; Task { await viewModel.sendLevels() } }
                    ), in: -60...12, step: 0.5)
                ...
                HStack {
                    Slider(value: Binding(
                        ...
                        set: { device.outputLevel = $0; Task { await viewModel.sendLevels() } }
                    ), in: -60...12, step: 0.5)
                ...
                Toggle("Mute Input", isOn: Binding(
                    ...
                    set: { device.inputMute = $0; Task { await viewModel.sendMutes() } }
                ))
                Toggle("Mute Output", isOn: Binding(
                    get: { device.outputMute },
                    set: { device.outputMute = $0; Task { await viewModel.sendMutes() } }
                ))
            }
            ...
            Section("Crossover") {
                ...
                HStack {
                    Slider(value: Binding(
                        ...
                        set: { device.crossoverFrequency = $0; Task { await viewModel.sendCrossover() } }
                    ), in: 20...200, step: 1)
                ...
                Picker("Slope", selection: Binding(
                    ...
                    set: { device.crossoverSlope = $0; Task { await viewModel.sendCrossover() } }
                )) {
                ...
                Section("Timing") {
                    ...
                    HStack {
                        Slider(value: Binding(
                            ...
                            set: { device.phaseDelay = $0; Task { await viewModel.sendPhase() } }
                        ), in: 0...20, step: 0.1)
                    ...
                    Toggle("Invert Polarity", isOn: Binding(
                        get: { device.polarity },
                        set: { device.polarity = $0; Task { await viewModel.sendPolarity() } }
                    ))
                }
            }
                ...
                ForEach(device.eqBands.indices, id: \.self) { idx in
                    HStack {
                        Toggle("Band \(idx + 1)", isOn: Binding(
                            ...
                            set: { device.eqBands[idx].enabled = $0; Task { await viewModel.sendEQBand(at: idx) } }
                        ))
                        Spacer()
                        ...
                        Slider(value: Binding(
                            ...
                            set: { device.eqBands[idx].frequency = $0; Task { await viewModel.sendEQBand(at: idx) } }
                        ), in: 20...20_000, step: 1)
                            .frame(width: 200)
                        ...
                        Slider(value: Binding(
                            ...
                            set: { device.eqBands[idx].gain = $0; Task { await viewModel.sendEQBand(at: idx) } }
                        ), in: -15...15, step: 0.5)
                            .frame(width: 160)
                        ...
                        Slider(value: Binding(
                            ...
                            set: { device.eqBands[idx].qFactor = $0; Task { await viewModel.sendEQBand(at: idx) } }
                        ), in: 0.1...10, step: 0.05)
                            .frame(width: 140)
                    }
                }
                ...
                Section("Limiter") {
                    ...
                    HStack {
                        Slider(value: Binding(
                            ...
                            set: { device.limiterThreshold = $0; Task { await viewModel.sendLimiter() } }
                        ), in: -20...12, step: 0.5)
